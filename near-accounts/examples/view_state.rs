...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Need to change as per the code
    let args_json = json!({"prefix": "contract.near-api-rs.testnet"});

    let prefix = serde_json::to_vec(&args_json)?.into();

    // Call create_account
    let result = account.view_state(prefix).await;
